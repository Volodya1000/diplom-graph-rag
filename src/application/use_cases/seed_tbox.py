...
class SeedTboxUseCase:
    ...
    async def execute(self, force: bool = False) -> int:
        ...
        classes_to_save = (
            list(BASE_TBOX_CLASSES) if force else [c for c in BASE_TBOX_CLASSES if c.name.lower() not in existing_names]
        )
        if classes_to_save:
        ...
        existing_keys = {
            (r.source_class.lower(), r.relation_name.upper(), r.target_class.lower()) for r in current_rels
        }
