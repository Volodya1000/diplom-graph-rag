...
class OntologyUpdateValidator:
    ...
    def validate_merge(
        ...
        for r in merged_relations:
            if r.source_class not in class_set or r.target_class not in class_set:
                errors.append(
                    f"Отношение {r.relation_name} ссылается на несуществующий класс"
                )
    ...
    def _merge_classes(
        self, current: List[SchemaClass], proposed: List[SchemaClass]
    ) -> List[SchemaClass]:
    ...
    def _merge_relations(
        self, current: List[SchemaRelation], proposed: List[SchemaRelation]
    ) -> List[SchemaRelation]:
        def key(r):
            return (
    ...
    def _has_cycle(self, classes: List[SchemaClass]) -> bool:
        graph: Dict[str, str] = {
            c.name.lower(): c.parent.lower() for c in classes if c.parent
        }
